...
package com.local.luna.jundi.jokerBot.states;

import com.bueno.spi.model.CardToPlay;
import com.bueno.spi.model.GameIntel;
import com.bueno.spi.model.TrucoCard;
import com.local.luna.jundi.jokerBot.HandClassification;
import com.local.luna.jundi.jokerBot.exceptions.WithoutCardsToPlayException;
...
import static com.local.luna.jundi.jokerBot.utils.RoundUtils.getRoundNumber;
import static com.local.luna.jundi.jokerBot.utils.RoundUtils.jokerBotStartsTheRound;
...
public sealed interface RoundState
    ...
    default int defaultRaiseResponse(GameIntel intel) {
        ...
        boolean myCardsAreExcellent = HandClassification.EXCELLENT.equals(getHandEvaluation(intel));
        boolean myCardsAreGood = HandClassification.GREAT.equals(getHandEvaluation(intel));
    ...
    default HandClassification getHandEvaluation(GameIntel intel) {
        ...
        if (average >= 10) return HandClassification.EXCELLENT;
        if (average >= 8) return HandClassification.GREAT;
        if (average >= 6 || getNumberOfManilhas().apply(intel.getCards(), intel.getVira()) > 0) return HandClassification.GOOD;
        if (average >= 4) return HandClassification.NOT_BAD;
        return HandClassification.BAD;
    }
    ...
    default boolean raiseHandByMyCards(GameIntel intel) {
        if (HandClassification.EXCELLENT.equals(getHandEvaluation(intel))) return true;
        return HandClassification.GREAT.equals(getHandEvaluation(intel));
    }
}
